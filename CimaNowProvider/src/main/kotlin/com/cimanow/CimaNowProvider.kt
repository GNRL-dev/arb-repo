...
class CimaNow : MainAPI() {
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        ...
        var title = select("li[aria-label=\"title\"]").text()
            .replace(" <em>.*|\\n".toRegex(), "")
            .replace("&nbsp;", "")
        ...
        val quality = select("li[aria-label=\"ribbon\"]").first()?.text()
            ?.replace(" |-|1080|720".toRegex(), "")

        val dubStatus = select("li[aria-label=\"ribbon\"]")
            .any { it.text().contains("مدبلج") }
        if (dubStatus) title = "$title (مدبلج)"

        return newMovieSearchResponse(
            name = "$title ${select("li[aria-label=\"ribbon\"]:contains(الموسم)").text()}",
            url = url,
            type = tvType,
        ) {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        if (paginationElement.isNotEmpty()) {
            ...
            if (max != null) {
                ...
                (2..max).toList().map { pageNum ->
                    app.get("$mainUrl/page/$pageNum/?s=$query").document
                        .select("section article a").map { element ->

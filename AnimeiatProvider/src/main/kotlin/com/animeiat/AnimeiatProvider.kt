...
class Animeiat : MainAPI() {
  ...
  override suspend fun load(url: String): LoadResponse {
    ...
    // 1. Hidden form → submit to get player page
    val form = doc.selectFirst("form[action][target=_blank]")
    if (form != null) {
        val action = form.attr("action")
        if (!action.isNullOrBlank()) {
            val postData = form.select("input[name]").associate { input ->
                input.attr("name") to input.attr("value")
            }

            // Submit POST → usually returns a redirect to the real player page
            val playerPage = app.post(
                url = fixUrl(action),
                data = postData
            ).document

            // 2. Look for iframe on the redirected page
            val iframe = playerPage.selectFirst("iframe[src]")?.attr("src")
            if (!iframe.isNullOrBlank()) {
                val fixedIframe = fixUrl(iframe)
                val resolved = loadExtractor(fixedIframe, data, subtitleCallback, callback)
                if (resolved) success = true
                else {
                    // Last fallback: raw <video> tags
                    val iframeDoc = app.get(fixedIframe).document
